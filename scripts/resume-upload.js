...
// Files to skip (already uploaded or too large)
const skipFiles = [
  'hero\\135161-761273563_medium.mp4',
  'music.mp3'
];

// Resume from this file index
const lastUploaded = 'people\\DSC07568.JPG';
...
// Check if file already uploaded to Cloudinary
async function isAlreadyUploaded(relativePath) {
  ...
  try {
    ...
    // Check if any resource matches this file
    return result.resources.some(r => {
      const cloudName = path.basename(r.public_id);
      return cloudName.startsWith(baseName);
    });
  } catch (error) {
    return false;
  }
...
async function optimizeImage(filePath) {
  ...
  if (size >= MAX_SIZE && imageExtensions.includes(ext)) {
    ...
    console.log(`  Optimized: ${(size / 1024 / 1024).toFixed(2)}MB → ${(newSize / 1024 / 1024).toFixed(2)}MB`);
    return tempFile;
  }
...
// Main function
async function main() {
  console.log('Resuming Cloudinary upload...\n');
  ...
  const lastIndex = allFiles.findIndex(f => {
    const rel = path.relative(publicDir, f);
    return rel === lastUploaded;
  });
  
  const filesToUpload = allFiles.slice(lastIndex + 1);
  console.log(`Resuming from file ${lastIndex + 2}/${allFiles.length}`);
  ...
  for (const file of filesToUpload) {
    ...
    if (skipFiles.includes(relativePath)) {
      console.log(`⊗ Skipping known large file: ${relativePath}\n`);
      uploadedFiles.push({ success: false, path: relativePath, skipped: true });
      continue;
    }
    
    // Check if already uploaded
    const alreadyUploaded = await isAlreadyUploaded(relativePath);
    if (alreadyUploaded) {
  ...
  if (fs.existsSync(mappingPath)) {
    try {
      existingMappings = JSON.parse(fs.readFileSync(mappingPath, 'utf-8'));
    } catch (e) {}
  }
  
  const finalMappings = { ...existingMappings, ...urlMappings };
  fs.writeFileSync(mappingPath, JSON.stringify(finalMappings, null, 2));
